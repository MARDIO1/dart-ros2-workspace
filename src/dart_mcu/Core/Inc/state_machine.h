...
enum E_Dart_State { Boot = 100, Protect = 101, Remote = 102, Match = 103 };

enum E_Match_Actions { Enter, Wait, Launch, Reload, Exit, Undefined };

enum E_ResetActionReturnState { Operating, Finished, Failed };

enum E_Gate_State { OPENED, CLOSED, OPERATING };
...
struct Dart_FSM : public FSM {
  ...
  // Public
  // 0: 空闲 1: 将Load电机拉到装填位置 2: 降下升降机 3: 放开扳机舵机 4:
  // 将Load电机拉到初始位置 5: 升起升降机
  // 这里要改，改成新的
...
enum class UpsideState { MovingUp, MovingDown, Idle };
