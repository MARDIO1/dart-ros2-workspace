...
#define AIR_SUPPORT_DATA                                                       \
    0X0205 // 空中支援时间数据，1Hz周期发送，只有空中机器人主控发送
#define HURT_DATA 0X0206  // 伤害状态数据，伤害发生后发送
#define SHOOT_DATA 0X0207 // 实时射击数据，子弹发射后发送
#define PROJECTILE_ALLOWANCE                                                   \
    0X0208 // 子弹剩余发送数，英雄、步兵、哨兵、空中发送，10Hz周期发送
...
#define ROBOT_INTERACTION_DATA                                                 \
    0X0301 // 机器人间交互数据，发送方触发发送，上限10Hz
#define CUSTOM_ROBOT_DATA                                                      \
    0X0302 // 自定义控制器交互数据接口，通过客户端触发发送，上限30Hz//
...
  bit 2：己方补给区的占领状态，1 为已占领（仅 RMUL
适用）
   ...
   bit 6-11：己方高地占领状态
 bit 6-7：己方环形高地的占领状态，1 为被己方占领，2
为被对方占领
 bit 8-9：己方梯形高地的占领状态，1 为被己方占领，2
为被对方占领  bit
10-11：己方梯形高地的占领状态，1为被己方占领，2为被对方占领
...
// 8. 飞镖状态信息：cmd_id (0x0105)。发送频率：1Hz
// 周期发送，发送范围：己方机器人。
typedef struct
{
    /*
...
typedef struct
{
    ...
    bit 0：哨兵机器人是否确认复活
     0
    表示哨兵机器人确认不复活，即使此时哨兵的复活读条已经完成  1
    表示哨兵机器人确认复活，若复活读条完成将立即复活 bit
    1：哨兵机器人是否确认兑换立即复活  0
    表示哨兵机器人确认不兑换立即复活；  1
    表示哨兵机器人确认兑换立即复活，若此时哨兵机器人
...
typedef struct
{
    ...
    /*note:小地图左下角为坐标原点，水平向右为 X 轴正方向，竖直向上为 Y
轴正方向。
显示位置将按照场地尺寸与小地图尺寸等比缩放，超出边界的位置将在边界处显示*/
    ...
    int8_t delta_x[49];
    // 路径点 y轴增量数组，单位：dm
    /*note:增量相较于上一个点位进行计算，共 49 个新点位，X 与 Y
     * 轴增量对应组成点位*/
    int8_t delta_y[49];
 ...
 * @note	None
 */
#define Verify_CRC_Check_Sum(__RECEIVEBUFFER__, __DATALENGTH__)                \
    (Verify_CRC8_Check_Sum(__RECEIVEBUFFER__, JUDGE_CRC8_OFFSET + 1) &&        \
     Verify_CRC16_Check_Sum(__RECEIVEBUFFER__,                                 \
                            __DATALENGTH__ + JUDGE_DATA_LENGTH(0)))
...
#define Append_CRC_Check_Sum(__TRANSMITBUFFER__, __DATALENGTH__)               \
    ...
    {                                                                          \
        ...
        Append_CRC16_Check_Sum(__TRANSMITBUFFER__,                             \
                               __DATALENGTH__ + JUDGE_DATA_LENGTH(0));         \
    } while (0U)
