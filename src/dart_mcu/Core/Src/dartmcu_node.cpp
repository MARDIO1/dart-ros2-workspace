...
void microros_node_task(void) {
  ...
  xTaskCreate(motor_controller::pid_control_task, "pid_control_task", 256, NULL,
              25, NULL);
  ...
  while (1) {
    ...
    case WAITING_AGENT:
      EXECUTE_EVERY_N_MS(1000,
                         state = (RMW_RET_OK == rmw_uros_ping_agent(50, 1))
                                     ? AGENT_AVAILABLE
                                     : WAITING_AGENT;);
    ...
    case AGENT_AVAILABLE:
      ...
      if (state == WAITING_AGENT) {
        ...
        soundEffectManager.addSoundEffect(BUZZER_NOTE(buzzer_plug_in), false,
                                          false);
    ...
    case AGENT_CONNECTED:
      ...
      if (state == AGENT_CONNECTED) {
        ...
        soundEffectManager.addSoundEffect(BUZZER_NOTE(buzzer_remove), false,
                                          false);
