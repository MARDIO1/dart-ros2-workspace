...
shared_ptr<soundEffect_t> SoundEffectManager::addSoundEffect(note_t *notes_,
                                                             size_t notes_size_,
                                                             bool emergency,
                                                             bool circulating,
                                                             bool add_rest)
...
SoundEffectManager::addSoundEffect(const note_t *notes_, size_t notes_size_,
                                   bool emergency, bool circulating,
                                   bool add_rest)
{
    auto se = make_shared<soundEffect_t>();
...
void SoundEffectManager::timer_callback(void *pvParameters)
{
    ...
    if (se->progress < total && (se->state == SoundEffectState::PLAYING ||
                                 se->state == SoundEffectState::CIRCULATING))
    {
        bool rest =
            se->add_rest && (se->progress & 1) != 0; // 判断是否为休止符阶段
        ...
        __HAL_TIM_SET_AUTORELOAD(mgr->timer_beep, dur_ms * 10 - 1);
    }
    else
    {
        // 播放完毕或循环处理
        ...
        else
        {
            ...
            // 移除当前并启动下一个
            mgr->soundEffects_queue.erase(mgr->soundEffects_queue.begin());
