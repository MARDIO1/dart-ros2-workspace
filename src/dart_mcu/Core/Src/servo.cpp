...
servo trigger_servo[8];

void servo::begin(TIM_HandleTypeDef *htim, uint32_t channel,
                  uint32_t timerClock, uint32_t minPulseWidth,
                  uint32_t maxPulseWidth, uint16_t minAngle, uint16_t maxAngle,
                  uint32_t pwm_period, uint32_t pwm_frequency,
...
void servo::enable() { pwm_.start(); }

void servo::disable() { pwm_.stop(); }

inline uint32_t servo::calCompareValue(uint16_t angle) {
  return (angle - minAngle_) * (maxPulseWidth_ - minPulseWidth_) /
             (maxAngle_ - minAngle_) +
         minPulseWidth_;
}