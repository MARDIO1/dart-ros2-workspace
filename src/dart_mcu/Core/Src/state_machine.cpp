    ...
    if (RC_Data.Switch_Left == RC_SW_UP) {
      ...
      int16_t base_velocity = 0;
      {
        ...
        case 2:
          base_velocity = -CONFIG_MOTOR_LOAD_OPERATION_VELOCITY_DOWNWARD;
          // 状态转移
          if ((RC_Data.ch3 < 950) | (motor_controller::MotorLoadController[0]
                                             .current_angle_with_rounds_ <=
...
public:
  ...
  void update(OpenFSM &fsm) const override {
    // 保持“遥控兜底 + 自动重装填”的状态切换策略。
    setNextStateByRemote(false, true);

       int16_t base_velocity = 0;
    auto moveLoadToTarget = [&](int target_angle) {
      const int angle_0 =
          motor_controller::MotorLoadController[0].current_angle_with_rounds_;
      const int angle_1 =
          motor_controller::MotorLoadController[1].current_angle_with_rounds_;
      const int err_0 = target_angle - angle_0;
      const int err_1 = target_angle - angle_1;
      const int err_avg = (err_0 + err_1) / 2;

      if (std::abs(err_0) <= kLoadPosTolerance &&
          std::abs(err_1) <= kLoadPosTolerance) {
        base_velocity = 0;
        return true;
      }
    // Load 使用速度控制（外层位置逻辑在本动作内实现）。
    motor_controller::MotorLoadController[0].set_state(
        motor_controller::E_PID_Velocity_Angle_Controller_State::
            VELOCITY_CONTROL);
    motor_controller::MotorLoadController[1].set_state(
    ...
    // ===== 新装填动作关键阈值与目标参数 =====
    // Load 到位容差（编码器角度单位）。
    constexpr int kLoadPosTolerance = 5;
    // DM 摆锤角度到位容差（单位：度）。
    constexpr float kDmToleranceDeg = 3.0f;
    // DM 安装角度目标（单位：度）。
    constexpr float kDmInstallAngleDeg = 0.0f;
    // DM 发射准备角度目标（单位：度）。
    constexpr float kDmLaunchAngleDeg = 45.0f;
    // Pitch 舵机安装角度（单位：舵机角度）。
    constexpr uint16_t kPitchInstallAngle = 90;
    // Pitch 舵机发射角度（单位：舵机角度）。
    constexpr uint16_t kPitchLaunchAngle = 40;

    // 按轮次控制三路闸门：
    // phase=0: 全开；phase=1: 关1路；phase=2: 关2路。
    auto setSolenoidByRound = [&]() {
      for (uint8_t i = 0; i < 3; ++i) {
        pneumatic::main_solenoid[i].on();
      }
      const uint8_t round_phase = msgDartStatus.dart_launch_process % 3;
      if (round_phase == 1) {
        pneumatic::main_solenoid[0].off();
      } else if (round_phase == 2) {
        pneumatic::main_solenoid[0].off();
        pneumatic::main_solenoid[1].off();
      }
    };

    // Load 两电机共同基准速度（再叠加同步控制器输出）。
    int16_t base_velocity = 0;

    // 让双 Load 朝目标位置移动：
    // - 到位：返回 true，并将基准速度置零；
    // - 未到位：根据误差分段降速，返回 false。
    auto moveLoadToTarget = [&](int target_angle) {
      const int angle_0 =
          motor_controller::MotorLoadController[0].current_angle_with_rounds_;
      const int angle_1 =
      ...
      int16_t vel = CONFIG_MOTOR_LOAD_OPERATION_VELOCITY_DOWNWARD;
      const int abs_err = std::abs(err_avg);
      if (abs_err < 5000) {
        vel = CONFIG_MOTOR_LOAD_OPERATION_VELOCITY_DOWNWARD / 4;
      } else if (abs_err < 20000) {
        vel = CONFIG_MOTOR_LOAD_OPERATION_VELOCITY_DOWNWARD / 2;
      }

      base_velocity = (err_avg >= 0) ? vel : -vel;
      return false;
    };

    // 当前 DM 角度（度）与当前时刻。
    const float dm_angle_deg = motor::MotorWindmill.getAngleDeg();
    const TickType_t now = xTaskGetTickCount();

    // 分阶段流程变量（0~7）。
    uint8_t &stage = fsm.custom<Dart_FSM>()->ActionRemoteandReload_Reload_State;

    switch (stage) {
    case 0:
      // 阶段0：Load 后移到“安装准备后方位置”。
      if (moveLoadToTarget(CONFIG_MOTOR_LOAD_ANGLE_POST_LOAD)) {
        stage = 1;
      }
      break;

    case 1:
      // 阶段1：DM 摆锤转到安装角；到位后下发 Pitch 安装角并启动计时。
      motor::MotorWindmill.setpos(kDmInstallAngleDeg / 180.0f * 3.14159265f);
      if (std::abs(dm_angle_deg - kDmInstallAngleDeg) < kDmToleranceDeg) {
    ...
    case 3:
      // 阶段3：Load 下压执行主动装填；到位后开启气泵并按轮次设置闸门。
      if (moveLoadToTarget(CONFIG_MOTOR_LOAD_ANGLE_DOWN)) {
        pneumatic::main_air_pump.on();
        setSolenoidByRound();
    ...
    case 5:
      // 阶段5：Load 前移到“接住”位置。
      if (moveLoadToTarget(CONFIG_MOTOR_LOAD_ANGLE_CATCH_FORWARD)) {
        stage = 6;
      }
      break;

    case 6:
      // 阶段6：DM 转到发射准备角，同时 Pitch 转到发射角。
      motor::MotorWindmill.setpos(kDmLaunchAngleDeg / 180.0f * 3.14159265f);
      trigger_servo[7].setAngle(kPitchLaunchAngle);
      if (std::abs(dm_angle_deg - kDmLaunchAngleDeg) < kDmToleranceDeg) {
        stage = 7;
      }
      break;

    case 7:
      // 阶段7：Load 后拉到发射下位；完成后切到下一动作（通常是等待/发射）。
      if (moveLoadToTarget(CONFIG_MOTOR_LOAD_ANGLE_LAUNCH_DOWN)) {
        fsm.nextAction();
      }
      break;
