...
void LED_Flow::begin()
{
    ...
    flowTaskHandle_ = xTaskCreateStatic(flowTask, "LED_Flow", flowStackSize,
                                        this, 18, flowStack_, &flowTaskBuffer_);
}
    ...
    while (true)
    {
        switch (led_flow->flow_state_)
        {
            case FLOW_NORMAL:
            {
                ...
                pointer = (pointer + 1) % 8;
                length = uint8_t(
                    2.0 * sin((xTaskGetTickCount() * 3.1415926 / 4000.0)) +
                    4.0);
                // 缓动效果，以6为周期正弦变化
                vTaskDelay(
                    sin((xTaskGetTickCount()) * 3.1415926 / 2000.0) * 20 + 100);
                break;
            }
            case FLOW_NONE:
            {
