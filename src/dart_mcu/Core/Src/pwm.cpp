...
void PWM::configure(TIM_HandleTypeDef *htim, uint32_t channel,
                    uint32_t timerClock, uint32_t period, uint32_t frequency)
{
    htim_ = htim;
    channel_ = channel;
...
void PWM::setDutyCycle(float dutyCyclePercent)
{
    ...
    setCompareValue(
        static_cast<uint32_t>((dutyCyclePercent / 100.0f) * getAutoReload()));
}
...
//     configure(htim_, channel_, timerClock_, formerAutoReload,
//     formerFrequency); updateTimer();
// }

uint32_t PWM::getFrequency() const
{
    return timerClock_ /
           (getAutoReload() * (__HAL_TIM_GET_ICPRESCALER(htim_, channel_) + 1));
}

uint32_t PWM::getAutoReload() const { return __HAL_TIM_GET_AUTORELOAD(htim_); }
